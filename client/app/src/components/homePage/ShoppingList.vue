                ...
                <div class="card rounded">
                    <div class="card-body p-4">
                        ...
                        <ul class="list-group rounded-0">
                            ...
                            <p class="text-center text-muted" v-show="!ingredients.length"> Your shopping list is empty
                            </p>
                            <li class="list-group-item border-0 d-flex ps-0" type="checkbox"
                                ...
                                <input class="form-check-input me-3" style="width:30px; height:30px" type="checkbox"
                                    value="" aria-label="..." />
                                <p class="pt-2">{{ item.name }}</p>
                                <p v-if="!item.store_has" class="font-weight-light text-end text-muted pt-2 px-1"> (item
                                    not
                                    available in store)
                                </p>
                        ...
                        <button v-show="!!ingredients.length" type="button" class="btn btn-success"
                            @click="clearIngredientList()">Clear List</button>
    ...
    methods: {
        ...
        removeIngredient(ingredient) {
            ...
            if (index > -1) { // only splice array when item is found
                this.ingredients.splice(index, 1); // 2nd parameter means remove one item only
            }

            // // currently only updates the local instance of the shopping list
            // if (this.ingredients === 1) {
            //     this.ingredients = []
            // } else {
